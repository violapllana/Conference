...
const createPost = async (req, res) => {
  try {
    ...
    res.status(400).json({ message: 'Gabim në krijimin e post-it', error: err });
  }
};
...
const getPosts = async (req, res) => {
  try {
    ...
    res.status(400).json({ message: 'Gabim në marrjen e post-eve', error: err });
  }
};
...
const updatePost = async (req, res) => {
  try {
    ...
    res.status(400).json({ message: 'Gabim në përditësimin e post-it', error: err });
  }
};
...
const deletePost = async (req, res) => {
  try {
    ...
    await post.destroy();
    res.status(200).json({ message: 'Posti u fshi me sukses', post: post });
  } catch (err) {
    res.status(400).json({ message: 'Gabim në fshirjen e post-it', error: err });
  }
};
